...
dose_levels=['d33']
lmbdchdiffs=[6,7]
compare_dose_levels=['d1','d4']

pat_id_arr_fname = f'/datastore01/user-storage/y.zezhang/2025_high_dose_project/document/final_test_pat_list_def.txt'
diseased_patients = np.loadtxt(pat_id_arr_fname, dtype = 'str', comments="#", delimiter=",", unpack=False)

pat_id_arr_fname = f'/datastore01/user-storage/y.zezhang/2025_high_dose_project/document/final_test_pat_list_hl.txt'
...
base_folder = '/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/neural_network_data/nn_pred'
full_view_folder='/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/neural_network_data/testing/test_total_DEMIST'
...
for compare_dose_level in compare_dose_levels:
    for lmbdchdiff_index in lmbdchdiffs:
        for dose_level in dose_levels:    
                            ...
                            for severity in severity_setting:                                             
                                if diag == 'diseased':
                                    for  di_item in diseased_patients:    
                                        ...
                                        SA_name=os.path.join(cur_path,f'recon_pat{patient}_{def_type}_{dose_level}_it8_b32_lmbdchdiff{lmbdchdiff_index}_lmbdmdiff0.img')
                                        ...
                                        full_view_cur_path=os.path.join(full_view_folder,patient)
                                        #full_view_cur_path=os.path.join(full_view_cur_path,CT_method)
                                        full_view_cur_path=os.path.join(full_view_cur_path,def_type)  
                                        
                                        full_view_SA_name=os.path.join(full_view_cur_path,f'recon_pat{patient}_{compare_dose_level}_it8_c30o5.img')
                                    ...
                                    for hl_item in healthy_patients:
                                        ...
                                        SA_name=os.path.join(cur_path,f'recon_pat{patient}_hl_{dose_level}_it8_b32_lmbdchdiff{lmbdchdiff_index}_lmbdmdiff0.img')
            ...
            output_file=f'{save_folder}/test_nn_lmbdchdiff{lmbdchdiff_index}_{dose_level}_vs_{compare_dose_level}_dose_nn.txt'

            with open(output_file, 'w') as f:
                
                f.write(f"Comparing groups:\nGroup A: {dose_level}\nGroup B: {compare_dose_level} dose_level \n")
