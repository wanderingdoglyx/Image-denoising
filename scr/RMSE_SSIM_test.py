...
for dose_level in dose_levels:    
                    ...
                    for severity in severity_setting:                                             
                        if diag == 'diseased':
                            for  di_item in diseased_patients:    
                                ...
                                full_view_cur_path=os.path.join(base_folder,patient)
                                #full_view_cur_path=os.path.join(full_view_cur_path,CT_method)
                                full_view_cur_path=os.path.join(full_view_cur_path,def_type)  
                                
                                full_view_SA_name=os.path.join(full_view_cur_path,f'recon_pat{patient}_d1_it8_c30o5.img')
    ...
    output_file=f'{save_folder}/test_{dose_level}_vs_full_dose.txt'

    with open(output_file, 'w') as f:
        
        f.write(f"Comparing groups:\nGroup A: {dose_level}\n Group B: full dose_level \n")
