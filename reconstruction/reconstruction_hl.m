...
does_level='dose_level_100';
% Set paths
base_folder ='/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/projection/hl_v2';
%save_folder = append('/data01/user-storage/y.zezhang/2024_subsample_project/mod_reconstruction/',num2str(does_level),'/CTAC');
save_folder = append('/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/reconstruction/',num2str(does_level),'/CTAC');
patient_list_folder='/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/projection/hl_v2/dose_level_100';
...
brench_folder_name=append(base_folder,'/',does_level);
...
for k = 1 : length(subFolders)
    ...
    for mode_idx = 1:length(mode)
        ...
        if mode_index==0 
            ...
            CT_image_file_name=append(patient_id,'.ict');
            CT_head_file_name=append(patient_id,'.hct');
            ...
            %system(['castor-recon_ESSE_v3 -df ',image_file_name_base,'.cdh -fout ',image_file_name_base,' -oit 8:8  -it 8:6 -dim 64,64,64 -vox 6.80,6.80,6.80 -conf /home/y.zezhang/castor/config -vb 1 -opti MLEM -proj classicSiddon -cdrf ',CDPR_file]);
            %system(['castor-recon_ESSE_v3 -df ',image_file_name_base,'.cdh -fout ',image_file_name_base,' -oit 8:8  -it 8:6 -dim 64,64,64 -vox 6.80,6.80,6.80 -atn ',CT_head_file_name,' -conf /home/y.zezhang/castor/config -vb 1 -opti MLEM -proj classicSiddon -cdrf /data01/user-storage/y.zezhang/2024_subsample_project/supporting_file_reconstruction/CDRP30.par']);
