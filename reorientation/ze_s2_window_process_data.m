function ze_s2_window_process_data(category, AC_method, ind_pat, patient_list,dose_level)
    ...
    %%
    seg_dir = '/datastore01/user-storage/y.zezhang/2025_high_dose_project/data'; 
    ...
    %base_res_dir  = ['/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/mod_SA_images/',AC_method,'/',category,'/'];
    base_res_dir  = ['/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/mod_SA_images/',AC_method,'/'];
    ...
    %%
    %sev_arr = [100,175,250,500];%total
    %ext_arr = [15,30,45,60,90];%total
    %loc_arr = {'da'; 'di'};%total

    sev_arr = [375,500];%total
    ext_arr = [30,60];%total
    loc_arr = {'da'; 'di'};%total
    ...
    if strcmp(category,'healthy')
        ...
        def_name='hl';
        %% read sa 3d image
        fname = fullfile(base_res_dir, pat_id, def_name, ['reoriented_',dose_level,'.img']);
        ...
        fname = [seg_dir,'/def_centroid/',pat_id,'/def_centroid_','dl2115','_mod.bin'];
        f = fopen(fname);
        if f < 0
            fname = [seg_dir,'/def_centroid/',pat_id,'/def_centroid_','dl2115','_mod_apr3.bin'];
            f = fopen(fname);
        end
        ...
        recon_sa = ze_process_overlapped_train(recon_sa, lv_mask, c_LV);

        cur_fold = fullfile(base_res_dir, pat_id,def_name);
        if ~isfolder(cur_fold); mkdir(cur_fold); end

        fname = fullfile(cur_fold, ...
                        ['reoriented_windowed_',dose_level,'.img']);
        ...
        fname = fullfile(cur_fold, ['reoriented_windowed_MO_',dose_level,'.img']);
        my_fwrite(fname, recon_sa, 'float32');
        ...
        for ind_loc = 1:length(loc_arr)
            ...
            for def_ext = ext_arr
                for def_sev = sev_arr
                    ...
                    %fname = [seg_dir,'/def_centroid/',pat_id,'/def_centroid_','dil2115','_mod.bin'];
                    fname = [seg_dir,'/def_centroid/',pat_id,'/def_centroid_',def_loc,'21',num2str(def_ext),'_mod.bin'];
                    f = fopen(fname);
                    if f < 0
                    fname = [seg_dir,'/def_centroid/',pat_id,'/def_centroid_',def_loc,'21',num2str(def_ext),'_mod.bin'];
                    f = fopen(fname);
