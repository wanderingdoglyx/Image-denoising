...
#=============================================================================================================================
def train_seg3D(base_folder, weights_name, loss_fn_name, dose_level, num_iter, bt_size, num_epochs, learning_folder, 
  ...
  label_data_folder=f'{section_folder_base}/training'
  data_folder = f'{section_folder_base}/training'
  data_folder_prev = f'{section_folder_base}/def_centroid'
  ...
  pat_id_arr = os.listdir('/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/neural_network_data/training')
  ...
  for hd in ['hl']:
    for ind_pat in range(num_data_dict[hd]['start_ind'], num_data_dict[hd]['end_ind'] + 1):
      if (ind_pat in remove_indices):
        ...
        continue
      cur_inp_file = f'{data_folder}/{pat_id_arr[ind_pat]}/{hd}/reoriented_windowed_d{dose_level}.img'
      cur_label_file = f'{label_data_folder}/{pat_id_arr[ind_pat]}/{hd}/reoriented_windowed_d100.img'
      ...
      for loc in loc_arr:
        for ext in ext_arr:
          ...
          for sev in sev_arr:
            ...
            cur_inp_file = f'{data_folder}/{pat_id_arr[ind_pat]}/{def_name}/reoriented_windowed_d{dose_level}.img'
            cur_label_file = f'{label_data_folder}/{pat_id_arr[ind_pat]}/{def_name}/reoriented_windowed_d100.img'
            cur_X = my_read_bin(cur_inp_file, 'float32', input_shape_orig)
            cur_Y = my_read_bin(cur_label_file, 'float32', input_shape_orig)
            cur_X = (cur_X - np.min(cur_X))/(np.max(cur_X) - np.min(cur_X))*sc_data
            cur_Y = (cur_Y - np.min(cur_Y))/(np.max(cur_Y) - np.min(cur_Y))*sc_data
