...
severity_setting=   [375]#[100,175,250] #[250,500]
...
pat_id_arr_fname = f'/datastore01/user-storage/y.zezhang/2025_high_dose_project/document/patient_list/patient_names_total_clear.txt'
...
base_folder = '/datastore01/user-storage/y.zezhang/2025_high_dose_project/data/neural_network_data/training_v3'
...
for dose_level in dose_levels:
          ...
          for severity in severity_setting: 
            if diag == 'diseased':         
              for index, di_item in enumerate(diseased_patients):
                ...
                cur_loc = np.fromfile(def_loc_fname, dtype = 'float32').astype(int) - shift_index #-1 #0 -based / but nn2D has 1 shift
                #cur_loc = np.fromfile(def_loc_fname, dtype = 'float32').astype(int)
  ...
  fname_tSN = f'{save_folder}/trainingdata_loc_{location_setting_combined}_sev_{severity_setting_combined}_ext_{extent_setting_combined}_{dose_level}_IO{isIO}_Ud{Ud}.txt'
