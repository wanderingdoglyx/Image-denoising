...
#severity=[0 ,175, 250, 330,375,1000]

extension=[60,90,120,15,30,45] #total
#extension=[30, 45,60,90] #total

location=[ 'I', 'A', 'L', 'S', 'TA', 'TI'] # total
#location=['A']
...
def initial_simind_pipline(patient_list,mode_index,extension_index,severity,location,gender_index,simind_collimator_index):
            ...
            #for extension_index in extension:
                #for location_index in location:
                    for pat_id in patient_list:
                        p = multiprocessing.Process(target=simind_exec3.simind_exec,args=(pat_id,mode_index,extension_index,severity_index,location,gender_index,simind_collimator_index))
                        if __name__ == "__main__":
                            p.start()
                            processes.append(p)
        ...
        for gender_index in gender:
            for pat_id in patient_list:
                for simind_collimator_index in simind_collimator:
                    initial_simind_pipline(patient_list,mode_index,'none','none','none',gender_index,simind_collimator_index) 
                 ...
                 for extension_index in extension:
                    for gender_index in gender:
                        for simind_collimator_index in simind_collimator:
                            ...
                            initial_simind_pipline(patient_list,mode_index,extension_index,severity,location_index,gender_index,simind_collimator_index)
