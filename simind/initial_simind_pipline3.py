...
def initial_simind_pipline(pat_id,num_hl,mode_index,extension_index,severity_index,location_index,gender_index,simind_collimator_index):
    
  
    if mode_index==0:
        for num_hl_index in range(num_hl):
                    p = multiprocessing.Process(target=simind_exec2.simind_exec,args=(pat_id,mode_index,num_hl_index,extension_index,severity_index,location_index,gender_index,simind_collimator_index))
        ...
        for gender_index in gender:
            for pat_id in patient_list:
                for simind_collimator_index in simind_collimator:
                    initial_simind_pipline(pat_id,num_hl,mode_index,'none','none','none',gender_index,simind_collimator_index) 
            ...
            for extension_index in extension:
                for location_index in location:
                    for gender_index in gender:
                        for simind_collimator_index in simind_collimator:
                            for pat_id in patient_list:
                                initial_simind_pipline(pat_id,num_hl,mode_index,extension_index,severity_index,location_index,gender_index,simind_collimator_index)
